...
| **Keepa API** | Amazonの実際の価格履歴を取得しChart.jsでグラフ表示。24時間キャッシュでトークンを節約し、API障害時はモックデータへ自動フォールバック。 |
...
- [API設計書](./docs/api-design.md) — エンドポイント一覧・認証認可フロー・Firestoreデータ構造・エラー規約
...
- 商品管理（一覧・検索・追加）
- 商品詳細（利益計算・Keepa連携による実際の価格履歴グラフ・プライベートメモ）
- 在庫管理・注文記録（売上/利益はダッシュボードにリアルタイム集計）
